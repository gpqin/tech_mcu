...
int SCM_Board_Init()
{
  /*Configure LED in SCM_EVB_BOARD*/
  SCM_LEDn_Init(LED2);
  SCM_LEDn_Init(LED3);
  SCM_LEDn_Init(LED4);
  SCM_LEDn_OFF(LED2);
  ...
  /*Configure Button in SCM_EVB_BOARD*/
  SCM_BUTTON_Init(BUTTON_WAKEUP); /*PA0*/   
  SCM_BUTTON_Init(BUTTON_USER_KEY); /*PC13*/
    return 1;
}
...
/**
  ...
  */
BUTTON_Type SCM_BUTTON_Press()
{
  static uint8_t Pressed = 1;
  /*Get Button state in SCM_EVB board*/
  if ((Pressed == 1) && ((SCM_BUTTON_State(BUTTON_WAKEUP) == Bit_SET ) || (SCM_BUTTON_State(BUTTON_USER_KEY) == Bit_SET )))
  {
    /*debounce*/
    Pressed = 0;
    //Delay_ms(10);
    if (SCM_BUTTON_State(BUTTON_WAKEUP) == Bit_SET)
      return BUTTON_WAKEUP;
    if (SCM_BUTTON_State(BUTTON_USER_KEY) == Bit_SET)
      return BUTTON_USER_KEY;
  }else if((SCM_BUTTON_State(BUTTON_USER_KEY) == Bit_RESET) && (SCM_BUTTON_State(BUTTON_WAKEUP) == Bit_RESET))
  {
    Pressed = 1;
  }
  return NO_BUTTON;
